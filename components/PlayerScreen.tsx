...
import { User, Loader, Check, X, Trophy, LogOut } from 'lucide-react';
...
const PlayerScreen: React.FC<PlayerScreenProps> = ({ state, playerId, onJoin, onAnswer, onBack }) => {
  ...
  // Handle auto-rejoin if persistent state existed (simplified for demo)
  useEffect(() => {
    if (currentPlayer) {
      setJoined(true);
    }
  ...
  // Local Timer Calculation for visual sync
  useEffect(() => {
    let timerId: any;
        ...
        <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleJoin} className="space-y-4">
            ...
            <button 
              type="submit" 
              disabled={!nameInput}
              className="w-full bg-indigo-600 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-indigo-700 active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              参加する
            </button>
          </form>
  ...
  if (state.gameState === GameState.LOBBY || state.gameState === GameState.SETUP) {
    return (
      <div className="h-full bg-indigo-500 flex flex-col items-center justify-center p-6 text-white text-center relative">
         <button onClick={onBack} className="absolute top-4 right-4 p-2 text-indigo-200 hover:text-white"><LogOut size={20}/></button>
         ...
         <div className="mt-8 px-6 py-2 bg-white/20 rounded-full font-mono">
           {currentPlayer?.name}
         </div>
      </div>
    );
  ...
  // --- 3. PLAYING: QUESTION & ANSWER BUTTONS ---
      ...
      <div className="h-full bg-slate-100 flex flex-col">
         ...
         <div className="bg-white p-6 shadow-sm border-b text-center">
            <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">第 {state.currentQuestionIndex + 1} 問</span>
                <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{Math.ceil(timeLeft)}秒</span>
            </div>
            <p className="text-slate-800 font-medium truncate opacity-50">ホストの画面を見てください！</p>
         </div>
  ...
  if (state.gameState === GameState.FINAL_RESULT) {
    ...
    return (
      <div className="h-full bg-slate-900 flex flex-col items-center justify-center p-6 text-white relative">
        <button onClick={onBack} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-white"><LogOut size={20}/></button>
